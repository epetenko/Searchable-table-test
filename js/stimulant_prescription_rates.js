...
// Leaflet Pt. 1 -- creating the map, pulling the Carto tilelayer and labels, and setting the viewpoint
    ...
    map.setView(new L.LatLng(44.001775, -72.721660), 7.3
        // isMobile? 7.5:8.5
    );
...
// queue for the town-by-town data
    queue()
        .defer(d3.json, "js/us-albers-counties.json")
        .defer(d3.csv, "data/stimulant_prescription_rates.csv")
        .await(ready);
...
// Need to read in the variables and make them into numbers in some cases
        ...
        data.forEach(function(d) {
            ...
            thirteen_rate[d.census2010] = +d.thirteen_rate;
            seventeen_rate[d.census2010] = +d.seventeen_rate;
...
// optional - create a legend with those colors - 
        var legend = d3.select('#legend-key')
            .append('ul')
...
// All this adds in the shapefiles for the town-by-town data. Can be re-jiggered to do county instead. 
        collection = topojson.feature(us, us.objects.collection)
        ...
        var feature = g.selectAll("path")
            .data(collection.features.filter(function(d) { return d.properties.state_fips == 50; }))
            .enter().append("path")
...
// A test for qualitative color categories -- this is where you would put the DemColor() if you have quantitative categories.
        feature.style("fill", function(d) {
            console.log(d.properties.id)
            console.log(thirteen_rate[d.properties.id])
               return DemColor(seventeen_rate[d.properties.id])
...
// CHANGE THIS NO. 7:
// The tooltip text
            div.html("<h2 class='town_name'>" + county[d.properties.id] + "</h2><div id='infobox'><table class='muni_elex'><tr class='notes'><td>2013: </td><td>" + thirteen_rate[d.properties.id] + " per 100</td></tr><tr class='notes'><td>2017: </td><td>" + seventeen_rate[d.properties.id] + " per 100</td></tr></table></div>")
            // div
...
// Filter buttons to only show positive towns
        d3.select("#Yes-button").on("click", function() {
            d3.select("#Yes-button").transition().style('background-color', "#bbbaba")
            d3.select("#No-button").transition().style('background-color', "#fff")
        ...
        d3.select("#No-button").on("click", function() {
            d3.select("#No-button").transition().style('background-color', "#bbbaba")
            d3.select("#Yes-button").transition().style('background-color', "#fff")

            feature
                .transition()
                .style("fill", function(d) {
                    return DemColor(thirteen_rate[d.properties.id])
                })
        });
